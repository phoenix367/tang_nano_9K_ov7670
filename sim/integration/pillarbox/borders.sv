...
localparam LCD_FRAME_HEIGHT = 8;

// Active width is derived by the DUT as OUTPUT_HEIGHT * (CAM_W / CAM_H); match
// that here so the test is self-consistent at any LCD_FRAME_HEIGHT.
localparam ACTIVE_WIDTH = (LCD_FRAME_HEIGHT * CAM_FRAME_WIDTH) / CAM_FRAME_HEIGHT;
localparam LEFT_BORDER  = (LCD_FRAME_WIDTH - ACTIVE_WIDTH) / 2;
localparam ACTIVE_END   = LEFT_BORDER + ACTIVE_WIDTH;
...
`endif
    ...
    for (row = 0; row < LCD_FRAME_HEIGHT; row = row + 1) begin
        ...
        if (row == 0 && R !== 0) begin
            $sformat(str, "row 0 should map to source row 0, got %0d", R);
            logger.error(module_name, str); `TEST_FAIL
        end
        if (R <= prev_R) begin
            $sformat(str, "row %0d source row %0d did not advance past %0d", row, R, prev_R);
            logger.error(module_name, str); `TEST_FAIL
        end

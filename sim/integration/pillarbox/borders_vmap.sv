...
localparam CROP_OFFSET  = LEFT_BORDER;   // masking: active source col = output col (read from col 0)
...
`endif
    ...
    for (i = 0; i < $size(data_items); i = i + 1)
        data_items[i] = (i / CAM_FRAME_WIDTH);   // encode source row
    ...
    for (row = 0; row < LCD_FRAME_HEIGHT; row = row + 1) begin
        ...
        for (col = 0; col < LCD_FRAME_WIDTH; col = col + 1) begin
            ...
            if (col < LEFT_BORDER || col >= ACTIVE_END) begin
                ...
                a = col - LEFT_BORDER;
                if (a == 0) begin
                    R = e;                       // source row of this output row
                    if (row == 0 && R !== 0) begin
                        $sformat(str, "row 0 maps to source row %0d, expected 0", R);
                        logger.error(module_name, str); `TEST_FAIL
                    end
                    if (R <= prev_R) begin
                        $sformat(str, "row %0d source row %0d did not advance past %0d (stuck/backwards vertical addressing)", row, R, prev_R);
                        logger.error(module_name, str); `TEST_FAIL
                    end
                    if (R >= CAM_FRAME_HEIGHT) begin
                        $sformat(str, "row %0d reads source row %0d, out of range (>=%0d)", row, R, CAM_FRAME_HEIGHT);
                        logger.error(module_name, str); `TEST_FAIL
                    end
                end else if (e !== R) begin
                    $sformat(str, "row %0d active[%0d]: source row %0d, expected %0d (active region must be one source row)", row, a, e, R);
                    logger.error(module_name, str); `TEST_FAIL
                end
            end
            idx = idx + 1;

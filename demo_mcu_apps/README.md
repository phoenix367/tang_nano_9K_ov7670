...
| [`calc`](calc/calc.c) | (C + libgcc) Host-driven **floating-point calculator**: the host sends `op, a, b` over the mailbox, the MCU computes in IEEE-754 single precision (`+ - * /`, `sqrt`, `1/x`, integer `pow`) and returns the result on the OSD + as raw bytes. SERV has no FPU → **libgcc soft-float**, so this needs the 16 KB MCU RAM build. Demonstrates host↔MCU comms. |

...
| [`roi_tm`](roi_tm/roi_tm.c) | (C) **Fixed-ROI face presence via a Tsetlin Machine** — the trained deployment of the `roi_collect` data. Featurizes the ROI (one "brighter than the ROI mean" bit per cell) and classifies with a TM whose clause masks are baked in from `tm_model.h`; inference is pure bitwise AND/compare/vote (no multiply, no libgcc). Train + export the model offline with [`train_tm.py`](roi_tm/train_tm.py); see [`roi_tm/README.md`](roi_tm/README.md) for the collect → train → deploy loop. Lights a "FACE" label; heartbeat 0xE0 = `bit7 present | (vote+64)`. |

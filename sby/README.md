...
| `formal_arbiter` (ctest, yosys SAT) | `src/arbiter.v` (width fixed to 4 via chparam) | sequential, k-induction | `grant` always one-hot (mutual exclusion, even mid-transition); a grant lane was requested last cycle; no grant unless `enable` was high; `select` indexes the granted lane |
...
| `formal_watchdog` (ctest, yosys SAT) | `src/watchdog.sv` (real params; cover shrinks them via chparam) | sequential, k-induction | `hang == OR(subsystem_hang)`; each subsystem-hang bit is sticky (no flapping); `monitoring` is sticky once armed; no hang before `monitoring` |
